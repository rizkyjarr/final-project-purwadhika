...
def fetch_and_map_rupiahcepat():
    ...
    if len(extracted_numbers) == len(column_headers):
        ...
        print("⚠️ Warning: Extracted values do not match expected number of columns!")
...
def upsert_to_bigquery():
    ...
    if job:
        print(f"⚠️ Error inserting rows into {table_ref}: {job}")
    else:
        print(f"✅ Data successfully upserted into {table_ref}")

# === FUNCTION 3: DAG EXECUTION FUNCTION === #
def run_pipeline():
    upsert_to_bigquery()
    print("🚀 Data scraping and upsert complete!")